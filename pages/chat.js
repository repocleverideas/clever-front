...
function chat(data) {
  ...
  return (
    <>
      <Head>
        <title>Chat</title>
        <meta name="description" content="Chat - Clever IDEAS" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <Header />

      <section className={styles.background} style={{background: data.hero.color && data.hero.color, paddingTop:'12rem'}}>
...
export async function getStaticProps({ locale }) {
  ...
  // const res = await fetch(`http://localhost:1337/chat-contact?_locale=${locale}`)
  const res = await fetch(`https://cleverideas-web.herokuapp.com/chat-contact?_locale=${locale}`)
